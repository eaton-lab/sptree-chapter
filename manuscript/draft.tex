...
\begin{figure}
	\centering
		...
		\caption{
			The effect of genetic linkage on the distribution of genealogies simulated on two species trees with different numbers of tips but the same probability of incongruence at each edge (all internode lengths are 0.5 coalescent units). 	% The genome-wide distribution of genealogies differs from that among neighboring regions of the genome where genealogies are correlated, and the difference is greater in trees with more sampled tips. (a-b) 
			% Linked or unlinked genealogies were simulated on two species trees the same probability of discordance between nodes (i.e., 		internode distances are the same in coalescent units). (c-f) 
...
\begin{figure}
	\centering
		\includegraphics[width=0.8\textwidth]{./figures/Fig-3-post2}
		\caption{
